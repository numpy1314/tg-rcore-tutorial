...
def read_archive_config(repository_root: Path, agent_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(
            "tg-rcore-session-archive: 无法读取项目归档配置，已跳过本次归档。",
            file=sys.stderr,
        )
        return None
    ...
    if not isinstance(mode, str) or mode not in SUPPORTED_MODES:
        print(
            "tg-rcore-session-archive: 归档等级无效，使用 messages。",
            file=sys.stderr,
        )
        mode = MODE_MESSAGES
...
def main() -> int:
    try:
        ...
        print(f"tg-rcore-session-archive: hook 未完成（{type(error).__name__}），请检查配置与源会话文件。", file=sys.stderr)
    return 0
