...
PLUGIN_NAME = "tg-rcore-session-archive"
MARKETPLACE_NAME = "tg-rcore-tutorial"
...
class Progress:
    ...
    def header(self, root, target):
        print(colored("\ntg-rCore · AI 过程记录配置", "1;36"), flush=True)
        self.info(f"项目：{root}")
...
def install_codex(root, progress=None):
    ...
    progress.step("注册本项目的 Codex 插件源", run, ["codex", "plugin", "marketplace", "add", str(root)], root)
...
def install_claude(root, progress=None):
    ...
    progress.step("注册本项目的 Claude Code 插件源", run, ["claude", "plugin", "marketplace", "add", "--scope", "user", str(root)], root)
