...
'''
    ...
    def test_invalid_config_is_not_replaced_and_installer_reports_failure(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            for source in ('{"broken":', '{"duplicate":1,"duplicate":2}', '[]', '{"chat.hookFilesLocations": false}'):
                ...
                self.assertFalse((root / ".vscode/tg-rcore-hooks").exists())
    ...
    def test_installer_matches_template_and_preserves_unrelated_hooks(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            for name in ("archive_session.py", "archive_storage.py", "copilot_hook.py"):
                self.assertEqual((SCRIPTS / name).read_bytes(), (root / ".vscode/tg-rcore-hooks" / name).read_bytes())
    ...
    def test_private_files_ignored_and_template_is_not(self):
        result = subprocess.run(["git", "check-ignore", ".vscode/settings.json", ".vscode/session-archive.json", ".vscode/tg-rcore-hooks/hooks.json", ".ai/agent-sessions/vscode-copilot/2026-09-09_00-00-00_session.jsonl"], cwd=REPOSITORY, text=True, capture_output=True)
