...
class CursorSetupTests(unittest.TestCase):
    ...
    def test_checked_in_hooks_match_installer_and_ignore_private_files(self):
        ...
        result = subprocess.run(["git", "check-ignore", ".cursor/session-archive.json", ".cursor/hooks.json", ".cursor/tg-rcore-hooks/cursor_hook.py", ".ai/agent-sessions/cursor/2026-09-09_00-00-00_session.jsonl", ".ai/agent-sessions/cursor/.state/session.sqlite3"], cwd=REPOSITORY, text=True, capture_output=True)
    ...
    def test_installed_hook_runs_without_plugin_source_after_branch_switch(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            for filename in ("cursor_hook.py", "archive_session.py", "archive_storage.py"):
                self.assertEqual((SCRIPTS / filename).read_bytes(), (root / ".cursor/tg-rcore-hooks" / filename).read_bytes())
