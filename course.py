...
MONITOR = ROOT / '.course-monitor'
VSIX = MONITOR / 'rewind-ide-0.3.0.vsix'
...
def check_repository():
    ...
    for name in ['.course-monitor', '.ai', '.ai/events', '.ai/ide', '.ai/submissions', '.ai/.gitignore']:
        path = ROOT / name
...
def install(skip_extension=False):
    ...
    if hook and hook != '.course-monitor/hooks':
    ...
    ignored = subprocess.run(['git','-C',str(ROOT),'check-ignore','-q','--no-index','.ai/submissions/course-check.jsonl'])
    if ignored.returncode == 0: raise RuntimeError('Repository ignore rules exclude .ai/submissions; adjust them first.')
    if ignored.returncode != 1: raise RuntimeError('Git ignore check failed')
    ...
    ignore=ROOT/'.ai/.gitignore'
    content=ignore.read_text(encoding='utf-8-sig') if ignore.exists() else ''
    extra=[line for line in ['events/','ide/','!submissions/'] if line not in content.splitlines()]
    if extra: ignore.write_text(content+('\n' if content and not content.endswith('\n') else '')+'\n'.join(extra)+'\n',encoding='utf8')
...
def open_workspace():
    ...
    workspace.write_text(json.dumps({'folders':[{'name':ROOT.name,'path':'../..'}],'settings':settings},ensure_ascii=False,indent=2),encoding='utf8')
