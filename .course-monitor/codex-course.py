...
from policy import redact, safe_directory, sanitize
...
def main():
    parser = argparse.ArgumentParser(description='Send one stdin prompt to Codex and log final course events. Requires an authenticated Codex CLI.')
    ...
    config = json.loads((root/'.course-monitor'/'config.json').read_text(encoding='utf8'))
