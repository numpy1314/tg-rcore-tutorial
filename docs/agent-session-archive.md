...
本项目提供 `tg-rcore-session-archive`，支持 Codex、Claude Code、Cursor 和 VS Code
...
需要 Bash、Python 3.9+，以及支持相应 hooks 的 Agent 客户端。在仓库根目录执行：
...
```bash
python3 plugins/tg-rcore-session-archive/scripts/setup_agents.py cursor
```

Codex / Claude Code 从本仓库的 marketplace 安装归档插件，在用户级配置中关闭该插件，
在项目级配置中启用。重新执行脚本会刷新本插件的缓存。首次使用 Codex 时还需在
`/hooks` 审阅并信任 hooks，然后从项目目录开启新会话；Claude Code 重新启动或执行
`/reload-plugins`。

Cursor / Copilot 使用原生项目 hooks，运行脚本分别安装到
`.cursor/tg-rcore-hooks/` 和 `.vscode/tg-rcore-hooks/`。安装会保留其他 hooks、编辑器设置
及 VS Code JSONC 注释。用 **Open Folder** 单独打开并信任本项目，再新建 Agent 对话；
已有窗口可重新加载。Cursor 查看 **Output → Hooks**，Copilot 查看
**Output → Copilot Chat Hooks**。多根工作区请改为单文件夹窗口。
...
本功能与现有 `course.py` 的 `.ai/events/` 实验操作日志独立。`course.py codex` 使用
临时会话模式；需要本插件的完整会话归档时，从项目根目录直接启动普通 Codex 会话。

## 验证

```bash
python3 -m unittest discover -s plugins/tg-rcore-session-archive/tests
